...
async def send():
    ...
    ctx.client_credentials[remote] = aiocoap.edhoc.EdhocCredentials(
            suite=2,
            method=3,
    ...
    await ctx.request(aiocoap.Message(code=aiocoap.PUT, uri=remote, payload=cbor2.dumps(encoded))).response_raising
